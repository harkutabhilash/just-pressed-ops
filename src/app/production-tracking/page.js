...
export default function ProductionTrackingPage() {
  ...
  useEffect(() => {
    ...
    setBottlingState({ seed: null, sku: null, qty: '' })
  }, [activeFlow])
  ...
  // ── SUBMIT: Bottling
  async function submitBottling() {
    if (!bottlingState.seed || !bottlingState.sku || !bottlingState.qty) return
    const db = sup()
    const time = nowIST()

    const { error } = await db.from('bottling_entries').insert({
      product_id: bottlingState.sku.product_id,
      sku_id: bottlingState.sku.sku_id,
      bottles_filled: parseInt(bottlingState.qty, 10),
      bottled_at_ist: time.formatted,
      bottled_at_epoch: time.epoch,
      created_by: session.user_id
    })

    if (error) {
      console.error('Bottling error:', JSON.stringify(error))
      setToast({ type: 'error', message: 'Failed to record bottling. Please try again.' })
      return
    }
    setToast({ type: 'success', message: `${bottlingState.qty} × ${bottlingState.sku.variant_name} bottled` })
    setBottlingState({ seed: null, sku: null, qty: '' })
  }
  ...
  const colorMap = {
    ...
    violet: { bg: 'bg-violet-50', border: 'border-violet-200', icon: 'bg-violet-100 text-violet-600', active: 'border-violet-500 bg-violet-50', badge: 'bg-violet-100 text-violet-700' },
  }
                    ...
                    {a.key === 'bottling' && (() => {
                      ...
                      return (
                        <>
                          ...
                          {/* Size grid — only shown once an oil is picked */}
                          {bottlingState.seed && (
                            sizesForOil.length === 0 ? (
                              <div className="bg-gray-50 border border-gray-200 rounded-xl px-3 py-4 text-center">
                                <p className="text-xs text-gray-500">No sizes available for {bottlingState.seed.name}</p>
                              </div>
                            ) : (
                              <div>
                                ...
                                <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                                  {sizesForOil.map(sku => (
                                    ...
                                    >
                                      ...
                                      {/* Bottle icon */}
                                      <div className="w-full aspect-square rounded-lg bg-violet-50 flex items-center justify-center mb-1.5">
                                        <svg className="w-6 h-6 text-violet-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 3h6v2a2 2 0 012 2v1a1 1 0 011 1v11a2 2 0 01-2 2H8a2 2 0 01-2-2V9a1 1 0 011-1V7a2 2 0 012-2V3z" />
                                        </svg>
                                      </div>
                                      <span className="text-xs font-semibold text-gray-800 block leading-tight">
                                        {sku.size_value} {sku.size_unit}
                                      </span>
                                      <span className="text-xs text-gray-500 block mt-0.5">{sku.sku_code}</span>
                                    </button>
                                  ))}
                                </div>
                              </div>
                          ...
                          {/* Bottle count — only shown once a size is picked */}
                          {bottlingState.sku && (
                            <>
                              <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-2">Number of Bottles</label>
                                <div className="flex gap-2">
                                  <input
                                    type="number"
                                    step="1"
                                    min="1"
                                    value={bottlingState.qty}
                                    onChange={e => setBottlingState(p => ({ ...p, qty: e.target.value }))}
                                    placeholder="0"
                                    className="flex-1 px-3 py-2.5 rounded-xl border-2 border-gray-200 text-sm font-medium outline-none focus:border-violet-400 transition-colors"
                                  />
                                  <span className="px-3 py-2.5 bg-gray-50 border-2 border-gray-200 rounded-xl text-sm font-semibold text-gray-600">bottles</span>
                                </div>
                              </div>

                              {/* Summary pill */}
                              <div className="bg-violet-50 border border-violet-200 rounded-xl px-3 py-2.5 flex items-center justify-between">
                                <span className="text-xs text-violet-600 font-medium">{bottlingState.sku.variant_name}</span>
                                <span className="text-xs font-semibold text-violet-700">MRP ₹{bottlingState.sku.mrp}</span>
                              </div>

                              <button
                                onClick={submitBottling}
                                disabled={!bottlingState.qty || parseInt(bottlingState.qty, 10) < 1}
                                className="w-full py-3 rounded-xl bg-violet-600 text-white text-sm font-bold transition-all duration-150 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-violet-700 active:scale-95"
                              >
                                Record Bottling
                              </button>
                            </>
